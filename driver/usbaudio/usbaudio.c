			       ...
			       unsigned int rate, unsigned int frame_bits)
{
	...
	for (i = 0; i < subs->nurbs; i++) {
		...
		printk("Data urb, interval:%d, number_of_packets:%d"
			"transfer_buffer_length:%d, in file:%s line:%d\n",
			1 << subs->syncinterval, u->urb->number_of_packets,
			u->urb->transfer_buffer_length, __FILE__, __LINE__);
	}
	...
	if (subs->syncpipe) {
		...
		for (i = 0; i < SYNC_URBS; i++) {
			...
			printk("sync urb, interval:%d, number_of_packets:%d"
				"transfer_buffer_length:%d, in file:%s line:%d\n",
				1 << subs->syncinterval, u->urb->number_of_packets,
				u->urb->transfer_buffer_length, __FILE__, __LINE__);
		}
