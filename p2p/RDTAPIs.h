...
struct st_RDT_Status
{
	...
	unsigned long BufSizeInSendQueue; //!< The size, in unit of byte, of buffer remaining in the sending queue in the local site
	unsigned long BufSizeInRecvQueue; //!< The size, in unit of byte, of buffer remaining in the receiving queue in the local site
};
 ...
 *				in all platforms.
 */
RDTAPI_API	void RDT_Set_Max_Channel_Number(unsigned long nMaxChannelNum);
 ...
 * \see RDT_Destroy()
 *
 * \attention The IOTC channel of specified channel ID will be turned on automatically
 *				by RDT_Create()
 */
