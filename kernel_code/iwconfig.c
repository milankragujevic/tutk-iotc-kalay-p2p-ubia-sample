...
int iwconfig_ra0(struct wireless_info *pstWireLessInfo)
{
	...
	rc = get_info(skfd, "ra0", pstWireLessInfo);
...
void iw_get_freq_str_value(char *buffer, int buflen, double freq)
{
    ...
    else
    {
        ...
        else
	    {
         ...
         snprintf(buffer, buflen, "%g", freq/divisor);
    }
}
...
void iw_get_wifi_signal(char *buffer)
{
	...
	int	nNoise = 0;
	double  rcpiqual = 0;
	char acStatsBuf[64] = {0};
	char acTmpebuf[4] = {0};
	struct wireless_info stWireLessInfo;

	//printf("iw_get_wifi_signal 111111111111\n");
	iwconfig_ra0(&stWireLessInfo);
	//printf("iw_get_wifi_signal 222222222222\n");

	if(stWireLessInfo.b.has_freq)
	{
		freq = stWireLessInfo.b.freq;	/* Frequency/channel */
		//printf("iw_get_wifi_signal 33333333333333\n");
	    iw_get_freq_str_value(buffer, 6, freq);
	    //printf("iw_get_wifi_signal 44444444444444\n");
	}

	if(stWireLessInfo.has_stats)
	{
		buffer[6] = (char)stWireLessInfo.stats.qual.qual;

		if(stWireLessInfo.stats.qual.level >= 64)
		{
			buffer[7] = stWireLessInfo.stats.qual.level - 0x100;
		}
		else
		{
			buffer[7] = stWireLessInfo.stats.qual.level;
		}

		if(stWireLessInfo.stats.qual.noise >= 64)
		{
			buffer[8] = stWireLessInfo.stats.qual.noise - 0x100;
		}
		else
		{
			buffer[8] = stWireLessInfo.stats.qual.noise;
		}
		//printf("level:%d, noise:%d\n", nLevel, nNoise);
	}

	printf("Freq:%s  qu: %d  sig: %d  noise: %d", buffer, (int)buffer[6], (int)buffer[7], (int)buffer[8]);
}
