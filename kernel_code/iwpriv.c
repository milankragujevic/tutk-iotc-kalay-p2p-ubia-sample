		...
		int		priv_num)	/* Number of descriptions */
{
  ...
  //printf("wfb add  come into set_private_cmd  444444//44444444444 buffer=%s\n", buffer);
  ...
  if((priv[k].get_args & IW_PRIV_TYPE_MASK) &&
	...
	case IW_PRIV_TYPE_CHAR:
	  ...
	  buffer[n] = '\0';
	  printf("%s\n", buffer);
	  //printf("wfb add  57575755  buffer = %s\n", buffer);
	  break;
	    ...
	    char *	ifname)		/* Dev name */
{
  ...
  /* Do it */
  printf("wfb add  come into set_private_cmd \n");
     ...
     char **	argv)
{
  ...
  //printf("iwpriv_main: argc = %d\n", argc);
  //printf("iwpriv_main: argv[0] = %s\n", argv[0]);
  //printf("iwpriv_main: argv[1] = %s\n", argv[1]);
  //printf("iwpriv_main: argv[2] = %s\n", argv[2]);
  //printf("iwpriv_main: argv[3] = %s\n", argv[3]);
...
/*
    ...
    nCmdType: 0: IWPRIV_CMD_CONNSTATUS
              1: IWPRIV_CMD_SET
*/
/**
  @brief iwpriv_func
...
int iwpriv_func(int nCmdType, char *pacName, char *pacParam1, char *pacParam2)
{
    ...
    if (IWPRIV_CMD_CONNSTATUS == nCmdType)
    {
    	//iwpriv ra0 connStatus
    	argv[1] = pacName;
    	argv[2] = "connStatus";
    	...
    	return iwpriv_main(3, argv);
    }
    else if (IWPRIV_CMD_SET == nCmdType)
    {
    	argv[1] = pacName;
    	argv[2] = "set";
    	argv[3] = pacParam1;
    	argv[4] = NULL;
    	return iwpriv_main(4, argv);
    }
    else
    {
    	printf("invalid cmd, if you need, at it in file : kernel_code/iwpriv\n");
    }
...
/**
  @brief iwpriv_func
  @author wfb
  @param[in] pacIfName:  35825
  @param[out] 无
  @return 无
  @remark 2013-11-25 增加该函数
  @note
 */
int iwpriv_connStatus(char *pacIfName)
{
	int skfd;		/* generic raw socket desc.	*/
	struct iwreq wrq;
    int goterr = 0;
    ...
    wrq.u.data.length = 1;
    wrq.u.data.flags = 4;
    wrq.u.data.pointer = (caddr_t)acBuf;

    strncpy(wrq.ifr_name, pacIfName, IFNAMSIZ);

    if(ioctl(skfd, 35825, &wrq) < 0)
    {
    	nRet = 0;
    }
    else
    {
    	if (NULL != strstr(acBuf, "Connected(AP"))
    	{
    		nRet = 1;
    	}
    	else
